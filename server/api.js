...
router.get('/checkid', function (req, res) {
    ...
    MongoClient.connect(url, { useNewUrlParser: true }, function (err, db) {
        ...
        dbo.collection('userinfo').find({ 'id': ua }).toArray(function (err, result) {
            if (err) throw err
            if (result.length === 0) {
                ap = { 'status': 0, 'dec': 'no such account' }
                res.send(ap)
            } else {
                ap = { 'status': 1, 'dec': 'this account exist', 'role': result[0].role }
                res.send(ap)
            }
            db.close()
        })
...
router.get('/checkuser', function (req, res) {
    MongoClient.connect(url, { useNewUrlParser: true }, function (err, db) {
        ...
        dbo.collection('userinfo').find({}, { id: 1, role: 1 }).toArray(function (err, result) {
            if (err) throw err
            if (result.length === 0) {
                ap = { 'status': 0, 'dec': 'no user' }
                res.send(ap)
            } else {
                ap = { 'status': 1, 'result': result }
                res.send(ap)
            }
            db.close()
        })
...
router.get('/login', function (req, res) {
    ...
    MongoClient.connect(url, { useNewUrlParser: true }, function (err, db) {
        ...
        dbo.collection('userinfo').find(logininfo).toArray(function (err, result) {
            if (err) throw err
            if (result.length === 0) {
                ap = { 'status': 0, 'dec': 'no such account', 'account': ua }
                res.send(ap)
            } else {
                ap = { 'status': 1, 'dec': 'login success', 'account': ua, 'role': result[0].role, 'avatar': result[0].avatar || null }
                res.send(ap)
            }
            db.close()
        })
    ...
    MongoClient.connect(url, { useNewUrlParser: true }, function (err, db) {
        ...
        dbo.collection('userinfo').find(isexist).toArray(function (err, result) {
            ...
            if (result.length === 0) {
                ...
                ap = { 'status': 1, 'dec': 'register success', 'account': ua, 'role': ur, 'avatar': avatar }
                res.send(ap)
            } else {
                ap = { 'status': 0, 'dec': 'account exist', 'account': ua }
                res.send(ap)
            }
            db.close()
        })
...
router.post('/posting', function (req, res) {
    ...
    MongoClient.connect(url, { useNewUrlParser: true }, function (err, db) {
        ...
        ap = { 'status': 1, 'dec': 'post success', 'r': userrole }
        res.send(ap)
        db.close()
    })
})
router.post('/deleteuser', function (req, res) {
    ...
    MongoClient.connect(url, { useNewUrlParser: true }, function (err, db) {
        ...
        ap = { 'status': 1, 'dec': 'delete success', 'account': ua }
        res.send(ap)
        db.close()
    })
})
router.post('/deletepost', function (req, res) {
    ...
    MongoClient.connect(url, { useNewUrlParser: true }, function (err, db) {
        ...
        ap = { 'status': 1, 'dec': 'delete success', 'timepoint': timepoint }
        res.send(ap)
        db.close()
    })
})
router.post('/changerole', function (req, res) {
    ...
    MongoClient.connect(url, { useNewUrlParser: true }, function (err, db) {
        ...
        ap = { 'status': 1, 'dec': 'updata success', 'account': ua }
        res.send(ap)
        db.close()
    })
})
router.post('/addreview', function (req, res) {
    ...
    MongoClient.connect(url, { useNewUrlParser: true }, function (err, db) {
        ...
        ap = { 'status': 1, 'dec': 'review success', 'ur': ua + '-' + rc }
        res.send(ap)
        db.close()
    })
})
router.post('/addzan', function (req, res) {
    ...
    MongoClient.connect(url, { useNewUrlParser: true }, function (err, db) {
        ...
        ap = { 'status': 1, 'dec': 'review success' }
        res.send(ap)
        db.close()
    })
})
